...
$ret="SELECT * FROM admins where (UserName=:uname ||  UserEmail=:uemail)";
...
{
// Query for Insertion
$sql="INSERT INTO admins(AID,FullName,UserName,UserEmail,UserMobileNumber,LoginPassword) VALUES(:aid,:fname,:uname,:uemail,:umobile,:upassword)";
...
<?php if($msg){ ?><div class="succWrap">
      ...
      <!-- Admin ID -->
      <label class="control-label" for="mobilenumber">Admin ID(AID)</label>
      <div class="controls">
        <input type="text" id="mobilenumber" name="aid" pattern="[0-9]{10}" maxlength="10"  title="Authorized Admin ID only"   class="input-xlarge" required>
        <p class="help-block">Be careful while filling the AID details</p>
      </div>
    </div>           
